...
for filetype_load, filetype in zip([loader.load_nirx, loader.load_snirf, loader.load_fif], ['NIRX', 'SNIRF', 'FIF']):
    ...
    montage = hrf.localize_hrfs(scans[0])
    ...
    montage.hbo_tree = hrf.tree(f"{TEST_DATA_DIR}/sham_hbo_hrfs.json")
    montage.hbr_tree = hrf.tree(f"{TEST_DATA_DIR}/sham_hbr_hrfs.json")
